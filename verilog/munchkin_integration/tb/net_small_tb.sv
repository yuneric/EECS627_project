...
dunkin_donuts #(
    ...
    initial begin
        //dump for verdi
        $fsdbDumpfile("net_small_tb.fsdb");
        $fsdbDumpvars(0, net_small_tb);
        $fsdbDumpMDA();
        ...
        // Load the mem files
        fmap_memory_file = "net_small_fmap.mem";
        wgt_memory_file = "net_small_wgt.mem";
        ...
        for (int j=(32'h00002000 >> 2); j<(32'h00006000 >> 2); j++) fmap_memory[j] = 0;
    ...
    always @(posedge clk_sys) begin
        if (rstn_sync && trap) begin
            ...
            output_fd = $fopen("net_small_fmap_dut.mem", "w");
            num_errors = 0;

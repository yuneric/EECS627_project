...
class LGNet():
    def __init__(self): 
        ...
        self.padding_l5 = 0
        self.scale_l5   = 2
        self.weights_l5 = np.random.randint(-4, 5, (192, 3, 3, 64), dtype=np.int8)
    ...
    def make_weight_mem_files(self, filename):
        with open(filename, 'w') as file:
            print(f'Weight layer 1 address start: 0x{weight_mem_base:08x} line #: {(weight_mem_base-weight_mem_base)//4+1:0d} Shape: {self.weights_l1.shape} Size: {self.weights_l1.nbytes / 1024:.2f} kB')
            layer1_address = self.write_mem(self.weights_l1, file) + weight_mem_base

            print(f'Weight layer 2 address start: 0x{layer1_address:08x} line #: {(layer1_address-weight_mem_base)//4+1:0d} Shape: {self.weights_l2.shape} Size: {self.weights_l2.nbytes / 1024:.2f} kB')
            layer2_address = self.write_mem(self.weights_l2, file) + layer1_address

            print(f'Weight layer 3 address start: 0x{layer2_address:08x} line #: {(layer2_address-weight_mem_base)//4+1:0d} Shape: {self.weights_l3.shape} Size: {self.weights_l3.nbytes / 1024:.2f} kB')
            layer3_address = self.write_mem(self.weights_l3, file) + layer2_address

            print(f'Weight layer 4 address start: 0x{layer3_address:08x} line #: {(layer3_address-weight_mem_base)//4+1:0d} Shape: {self.weights_l4.shape} Size: {self.weights_l4.nbytes / 1024:.2f} kB')
            layer4_address = self.write_mem(self.weights_l4, file) + layer3_address

            print(f'Weight layer 5 address start: 0x{layer4_address:08x} line #: {(layer4_address-weight_mem_base)//4+1:0d} Shape: {self.weights_l5.shape} Size: {self.weights_l5.nbytes / 1024:.2f} kB')
            layer5_address = self.write_mem(self.weights_l5, file) + layer4_address

    def make_fmap_mem_files(self, filename):
        with open(filename, 'w') as file:
            print(f'Input  fmap    address start: 0x{input_mem_base:08x} line #: {(input_mem_base-input_mem_base)//4+1:0d} Shape: {self.input_image.shape} Size: {self.input_image.nbytes / 1024:.2f} kB')
            input_address = self.write_mem(self.input_image, file) + input_mem_base

            print(f'Output layer 1 address start: 0x{input_address:08x} line #: {(input_address-input_mem_base)//4+1:0d} Shape: {self.output_l1.shape} Size: {self.output_l1.nbytes / 1024:.2f} kB')
            layer1_output_address = self.write_mem(self.output_l1, file) + input_address

            print(f'Output layer 2 address start: 0x{layer1_output_address:08x} line #: {(layer1_output_address-input_mem_base)//4+1:0d} Shape: {self.output_l2.shape} Size: {self.output_l2.nbytes / 1024:.2f} kB')
            layer2_output_address = self.write_mem(self.output_l2, file) + layer1_output_address

            print(f'Output layer 3 address start: 0x{layer2_output_address:08x} line #: {(layer2_output_address-input_mem_base)//4+1:0d} Shape: {self.output_l3.shape} Size: {self.output_l3.nbytes / 1024:.2f} kB')
            layer3_output_address = self.write_mem(self.output_l3, file) + layer2_output_address

            print(f'Output layer 4 address start: 0x{layer3_output_address:08x} line #: {(layer3_output_address-input_mem_base)//4+1:0d} Shape: {self.output_l4.shape} Size: {self.output_l4.nbytes / 1024:.2f} kB')

...
def generate_test_case(test_id, dim, act_range, weight_range,
                       relu_en, shift_by, maxpool_en, out_dir):
    """Generate one test case: stimulus files + golden output."""

    np.random.seed(42 + test_id)

    A = np.random.randint(act_range[0], act_range[1] + 1, size=(dim, dim)).astype(np.int64)
    W = np.random.randint(weight_range[0], weight_range[1] + 1, size=(dim, dim)).astype(np.int64)
    ...
    with open(f"{prefix}_config.hex", 'w') as f:
        f.write(f"{dim:02x}\n")
        f.write(f"{int(relu_en):02x}\n")
        f.write(f"{shift_by:02x}\n")
        f.write(f"{int(maxpool_en):02x}\n")
        f.write(f"{num_output_rows:02x}\n")
